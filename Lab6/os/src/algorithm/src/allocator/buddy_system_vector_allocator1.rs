...
impl BuddySystemVectorAllocator {
    fn update (&mut self, new_block: Block, index: usize) {
        if self.list[index].is_empty() {
            self.list[index].push_back(new_block);
        }
        else {
            let mut temp_vec = Vec::new();
            while let Some(temp_block) = self.list[index].pop_back() {
...
impl VectorAllocator for BuddySystemVectorAllocator {
    fn new(capacity: usize) -> Self {
        ...
        let mut new_list = vec![LinkedList::new(); list_size];
        let first_block = Block {
            start: 0,
            size: total_size,
        };
        new_list[list_size-1].push_back(first_block);
        Self {
            capacity: total_size,
            list_size: list_size,
            list: new_list,
    ...
    fn alloc(&mut self, size: usize, _align: usize) -> Option<usize> {
        ...
        if find_index > self.list_size - 1 {
            ...
            for i in 0.. (find_index- get_index) {
                let temp_block = self.list[find_index - i].pop_back();
                match temp_block {
                    None => panic!("the linkedlist is empty...from buddy system"),
                    Some(tblock) => {
                        self.list[find_index - i - 1].push_back(
                            Block {
                                start: tblock.start,
                                size: tblock.size / 2,
                            }
            ...
            let alloc_block = self.list[get_index].pop_back();
            match alloc_block {
